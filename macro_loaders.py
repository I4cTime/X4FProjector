...
def macro_parser(macro_id, macro_type, prop_node, lresolver):
    ...
    if macro_type.startswith('ship_'):
        ...
        props['hull'] = int(get_xpath_attrib(prop_node, './hull', 'max', 0))
        ...
        props['mass'] = float(get_xpath_attrib(prop_node, './physics', 'mass', 0))
        ...
        props['dock_external'] = int(dock.get('external', 0))
        ...
        props['speed'] = int(float(bullet.get('speed', 0)))
...
def ship_loader(floader, lresolver, macro_db, ext_name):
    ...
    for ship_size in ['xs', 's', 'm', 'l', 'xl']:
        ...
        try:
            ...
            LOG.warning('Failed to parse ship_loader: %s', e)
...
def shield_loader(floader, lresolver, macro_db, ext_name):
    ...
    try:
        ...
        LOG.warning('Failed to parse shield_loader: %s', e)
...
def engine_loader(floader, lresolver, macro_db, ext_name):
    ...
    try:
        ...
        LOG.warning('Failed to parse engine_loader: %s', e)
...
def weapon_loader(floader, lresolver, macro_db, ext_name):
    ...
    try:
        ...
        LOG.warning('Failed to parse weapon_loader: %s', e)
...
def missilelauncher_loader(floader, lresolver, macro_db, ext_name):
    ...
    try:
        ...
        LOG.warning('Failed to parse missilelauncher_loader: %s', e)
